# Personal triage, the shared standard

What counts as "needs my attention" across email, Slack, Jira, and calendar, and how to rank and trim it. This is the standard the `daily_personal_brief` task runs against. It contains no personal values. Handles, important senders, channels, and timezone come from the task's `config.md`. The rules below are the agreed definition of attention, so a change here updates every personal brief at once.

**Scope note.** This is a *personal* triage standard, not a team or leader standard. It lives in the engineering domain because the roadmap files its consuming task here. If a second personal-triage task lands in another domain, this file graduates to a cross-cutting location. Where it covers Jira, it reuses `standards/engineering/definitions.md` rather than redefining anything.
...
**Jira.** Apply the Blocked, At risk, and Stale classification and thresholds from `standards/engineering/definitions.md` (at risk window 2 business days, stale threshold 3 business days), scoped to issues assigned to me. Reference those thresholds by pointer; do not restate the numbers here, so a change to definitions.md propagates automatically. Only those three buckets carry over. Explicitly exclude Completed yesterday (not actionable), Net new, and Needs escalation (leader concepts, wrong audience for a personal brief).

Classification reads the Jira status mapping from the shared `config/config.md`, the same mapping the team report uses. The personal query shape is assignee-scoped and lives here, not in definitions.md, because an assignee filter is personal-triage-specific:
